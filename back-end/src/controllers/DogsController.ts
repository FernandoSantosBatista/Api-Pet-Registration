...
import UploadLogoOfDogsService from 'src/services/UploadLogoOfDogsService';
...
class DogsController {
  ...
  public async create(request: Request, response: Response): Promise<Response> {
    const { name, species, sex, age, size } = request.body;
    const dogsRepository = new DogsRepository();
  ...
  public async uploadLogo(
    ...
    const { id } = request.params;
    const dogsRepository = new DogsRepository();
    const service = new UploadLogoOfDogsService(dogsRepository);

    const dogs = await service.execute({
      id,
      logo: request.file?.filename,
    });
